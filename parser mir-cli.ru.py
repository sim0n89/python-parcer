...
def get_pages_count(html):
    ...
    try:
        pagination = soup.find_all('ul', class_='pagination')[2].find('a').get('href').split('=')[1]
        print (pagination)
    except:
        pagination = 0
    if pagination:
...
def get_all_links (html):
    soup = BeautifulSoup (html, 'lxml')
    divs = soup.find('section', id='section-products').find_all('div', class_='product-content')
...
def get_page_data (html):
    soup = BeautifulSoup(html, 'lxml')
    try:
        cats = soup.find('ul', class_='main-menu-breadcrumbs').find_all('span', itemprop='name')
    except:
        cats = ''
    try:
        atrs = soup.find('div', id='product-overview-2').find_all('span', class_='main')
    except:
        atrs = ''
    try:
        img = HOST + soup.find('div', id='fotorama-product').find_next('img').get('src')
    except:
        img = ''
    try:
        product_prefix = soup.find('h1', itemprop='name').find_next('span', class_='product-prefix').text.strip()
        product_name = soup.find('h1', itemprop='name').find_next('span', class_='product-name').text.strip()
    ...
    try:
        price = soup.find('div', class_='block-price').find_next('span', class_='price').text.strip(' руб')
    except:
         price = ''
...
def main():
    ...
    for url in urls:
        ...
        for pages in range(1, pages_count + 1):
            ...
            all_links = get_all_links(get_html(url, params={'page': pages})) #передает адреса страниц с параметрами номеров
